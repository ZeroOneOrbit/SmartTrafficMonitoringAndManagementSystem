    ...
    <nav className="fixed top-0 left-0 z-50 w-full border-b border-cyan-500/20 bg-[#030712]/10 backdrop-blur-xl">
      <div className="mx-auto flex h-14 w-full max-w-7xl items-center justify-between px-3 sm:h-16 sm:px-6">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          <Link to={ROUTES.portal} className="flex items-center gap-2 rounded-lg border border-cyan-500/30 px-3 py-2 text-sm text-cyan-400 transition hover:bg-cyan-500 hover:text-black">
            <LogIn size={16} />
            লগইন
          </Link>

          <Link to={ROUTES.register} className="flex items-center gap-2 rounded-lg bg-cyan-500 px-3 py-2 text-sm font-semibold text-black transition hover:bg-cyan-400">
            <UserPlus size={16} />
            রেজিস্ট্রেশন
          </Link>
      ...
      >
        <div className="border-t border-cyan-500/20 bg-[#08111f]/95 px-3 py-3 backdrop-blur-xl">
          ...
          <div className="mt-4 space-y-2">
            ...
            <Link to={ROUTES.portal} onClick={() => setMenuOpen(false)} className="flex w-full items-center justify-center gap-2 rounded-lg border border-cyan-500/30 py-2 text-sm text-cyan-400 transition hover:bg-cyan-500 hover:text-black">
              <LogIn size={16} />
              লগইন
            </Link>

            <Link to={ROUTES.register} onClick={() => setMenuOpen(false)} className="flex w-full items-center justify-center gap-2 rounded-lg bg-cyan-500 py-2 text-sm font-semibold text-black transition hover:bg-cyan-400">
              <UserPlus size={16} />
              রেজিস্ট্রেশন
            </Link>
